...
const isAdmin = async (req, res, next) => {
  if (req.user.role !== "admin") {
    ...
    return res
      .status(403)
      .json({ error: "Access denied: Security incident logged." });
  }
  next();
};
...
router.post("/security/block-ip", authMiddleware, isAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: "IP address and associated account blocked successfully",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const update =
        type === "logins" ? { loginAttempts: 0 } : { apiRequestsCount: 0 };
...
router.put("/users/:id", authMiddleware, isAdmin, async (req, res) => {
  try {
    const { name, email, role, phone, city, isActive, isBlocked, password } =
      req.body;
...
router.put("/restaurants/:id", authMiddleware, isAdmin, async (req, res) => {
  try {
    ...
    const restaurant = await Restaurant.findByIdAndUpdate(
      req.params.id,
      { name, description, address, city, phone, isActive },
      { new: true },
    );
    res.json(restaurant);
  } catch (err) {
...
router.get("/feedbacks", authMiddleware, isAdmin, async (req, res) => {
  try {
    const generalFeedbacks = await Feedback.find()
      ...
      .sort({ createdAt: -1 });
    const orderReviews = await Order.find({
