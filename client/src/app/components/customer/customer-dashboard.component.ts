...
export class CustomerDashboardComponent implements OnInit, OnDestroy {
  ...
  subCheckout: any = {
    startDate: "",
    endDate: "",
    preferredMealTimings: "12:30 PM - 01:30 PM",
    deliveryAddress: "",
    phone: "",
  ...
  slides = [
    ...
    },
    {
      ...
      title2: "Pizzas",
      description:
        "Authentic Italian pizzas baked in a traditional woodfired oven.",
      price: "Rs. 2100",
      image:
  ...
  ngOnInit() {
    ...
    this.authService.currentUser.subscribe((user) => {
      ...
      if (user) {
        ...
        if (user.address)
          this.selectedAddress = `${user.address}, ${user.city || ""}`.replace(
            /,\s*$/,
            "",
          );
  ...
  markAsRead(notification: any) {
    ...
    this.authService
      .markNotificationAsRead(notification._id, "customer")
      .subscribe({
        next: (updated: any) => {
  ...
  pollTrackingOrder() {
    ...
    this.customerService.getOrderDetails(this.trackingOrder._id).subscribe({
      ...
      error: (err) =>
        console.error("Error polling order tracking details", err),
    });
  }
  ...
  get deliveryFee(): number {
    return this.cart.length > 0 ? 150.0 : 0; // Set flat delivery fee to 150 PKR
  }
  ...
  updateAddress() {
    ...
    if (this.newAddress.trim()) {
      const cityChanged =
        this.previousCity && this.previousCity !== this.selectedCity;
      this.previousCity = this.selectedCity;
      ...
      if (this.isLoggedIn) {
        ...
        if (!this.profileForm.savedAddresses)
          this.profileForm.savedAddresses = [];

        // Add to saved addresses if not already there
        const exists = this.profileForm.savedAddresses.find(
          (a: any) =>
            a.city === this.selectedCity && a.fullAddress === this.newAddress,
        );
        if (!exists) {
      ...
      next: (data: any) => {
        ...
        if (data.length > 0) {
          const sum = data.reduce(
            (acc: number, curr: any) => acc + curr.review.rating,
            0,
          );
  ...
  get hasInvalidCartItems(): boolean {
    return this.cart.some(
      (item) => item.cityAddedFrom && item.cityAddedFrom !== this.selectedCity,
    );
  }
  ...
  placeOrder() {
    ...
    const order = {
      ...
      restaurantId:
        ...
        this.cart[0]?.restaurantId?._id ||
        this.cart[0]?.restaurantId,
  ...
  submitReview() {
    ...
    this.customerService
      ...
      .subscribe({
        next: (response: any) => {
          alert(
            "Thank you! Your rating and review was submitted successfully.",
          );
          this.loadMyOrders();
          this.closeReviewModal();
          if (
            this.trackingOrder &&
            this.trackingOrder._id === this.reviewOrderId
          ) {
        ...
        error: (err: any) => {
          alert(
            "Error submitting review: " + (err.error?.error || "Unknown error"),
          );
        },
      });
  }
  ...
  loadSubscriptionRestaurants() {
    this.subscriptionService
      .getRestaurants({ city: this.selectedCity, ...this.subFilter })
      .subscribe({
        next: (res) => {
          this.subscriptionRestaurants = res.restaurants || [];
        },
        error: (err) =>
          console.error("Error loading subscription restaurants", err),
      });
  }
  ...
  selectSubscriptionRestaurant(restaurant: any) {
    ...
    this.subscriptionService.getRestaurantDetails(restaurant._id).subscribe({
      ...
      error: (err) =>
        console.error("Error loading restaurant subscription details", err),
    });
  }
  ...
  toggleSubExtra(extra: any) {
    const existingIndex = this.subCheckout.selectedExtras.findIndex(
      (e: any) => e.extraItemId === extra._id,
    );
    if (existingIndex > -1) {
  ...
  isSubExtraSelected(extraId: string): boolean {
    return this.subCheckout.selectedExtras.some(
      (e: any) => e.extraItemId === extraId,
    );
  }
  ...
  calculateSubGrandTotal(): number {
    ...
    const base =
      this.selectedSubPlan.discountedPrice ||
      this.selectedSubPlan.totalPrice ||
      0;
    const days = this.selectedSubPlan.planType === "weekly" ? 7 : 30;
    const extrasTotal = this.subCheckout.selectedExtras.reduce(
      (sum: number, e: any) => {
        const mult =
          e.repeatMode === "daily"
            ? days
  ...
  submitSubscriptionRequest() {
    ...
    this.subscriptionService.subscribe(payload).subscribe({
      next: (res) => {
        alert(
          "🎉 Subscription request submitted successfully! Awaiting restaurant approval.",
        );
      ...
      error: (err) =>
        alert(
          "Error submitting subscription: " + (err.error?.error || err.message),
        ),
    });
  }
  ...
  openSubActionModal(sub: any, actionType: string) {
    ...
    this.subActionData = {
      vacationDatesStr: "",
      newTiming: sub.preferredMealTimings,
      newAddress: sub.deliveryAddress,
      note: "",
  ...
  confirmSubAction() {
    ...
    if (this.subActionType === "vacation") {
      const dates = this.subActionData.vacationDatesStr
        .split(",")
        .map((d: string) => d.trim())
        .filter(Boolean);
      if (!dates.length) {
    ...
    this.subscriptionService
      .updateSubscriptionAction(this.selectedSubscriptionForAction._id, payload)
      .subscribe({
        next: (res) => {
        ...
        error: (err) =>
          alert(
            "Error updating subscription: " + (err.error?.error || err.message),
          ),
      });
  }
